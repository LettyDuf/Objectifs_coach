...
export function HandCard({
  ...
  return (
    <button
      type="button"
      className={`puzzle-card puzzle-card--${block.category}${isTrap ? " puzzle-card--trap" : ""}`}
      onClick={onPlay}
      aria-label={`Jouer la carte « ${label} »${isTrap ? " (carte à risque)" : ""}`}
      title={isTrap ? "Carte à risque : pose-la si tu veux, l'outil signalera le piège." : undefined}
    >
      {isTrap && (
