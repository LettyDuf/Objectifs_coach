...
## D39 — Purge complète de « goal » dans le corps des fiches Sprint
- Contexte : après D38 (renommage des 4 titres), Lætitia a repéré en usage réel (capture d'écran du quiz Anti-patterns sur la fiche « L'objectif recyclé ») que le mot « goal » restait massivement présent dans le corps des fiches — heroPhrase, texte courant, bullets, notes, `practiceCtaLabel` — ce que j'avais provisoirement mis en dette séparée (D38) en sous-estimant qu'elle voulait le voir traité maintenant, pas plus tard.
- Choix : 39 remplacements ciblés dans `src/content/sheets/sprint.dev.fr.ts` (« goal » → « objectif », avec gestion manuelle des élisions et contractions françaises : « le goal » → « l'objectif », « du goal » → « de l'objectif », « au goal » → « à l'objectif », « nouveau goal » → « nouvel objectif » devant voyelle, « ce goal » → « cet objectif »). Les citations littérales en anglais (Scrum Guide, Scrum.org, Mike Cohn, Kniberg) qui utilisent « Goal » en tant que terme cité restent inchangées, conformément à D30 (citations en VO + traduction entre crochets) — le remplacement n'a porté que sur le « goal » minuscule en prose française, jamais sur les citations.
- Hors périmètre, assumé : les identifiants internes (`sprint.sheet.vanity-goal`, `sprint.sheet.single-goal`, etc.) gardent « goal » en anglais — ce sont des clés de code invisibles à l'utilisatrice, pas du contenu affiché. Les renommer casserait des références croisées (tests, `PitfallQuizCase.id` dérivé) pour un gain nul côté utilisatrice. Pas une exception à D30 (qui porte sur le vocabulaire affiché), une distinction domaine/infrastructure.
- Alternative écartée (traiter au fil de l'eau, dette qui ne se referme jamais) : c'est ce que j'avais proposé en D38, corrigé ici — un mot d'une doctrine appliquée à moitié est pire qu'une doctrine non appliquée, ça sème le doute sur tout le reste du corpus francisé.
